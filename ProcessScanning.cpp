...
int showpresent(DLinkList& L, DLinkList LL, LinkList& Lt)//显示当前进程 
{
	int flag = 0;
...
int showend(LinkList& L, DLinkList LL, DLinkList Lt)//显示结束进程 
{
	int i = 0;
	int flag = 0;
...
int circuit(LinkList& L, DLinkList L1)//根据present删除结束进程 
{
	DLNode* temp = L1->next;
	while (temp)
	{
...
int circuitll(DLinkList& pre, DLinkList now)//根据现在校验基准组 
{
	vector<char*> prev;
...
int main()
{
	DLinkList L, LL;//LL-pre,L-now（LL为基准组
	LinkList L2;//L2-end
	InitDList(L);
	InitDList(LL);
	InitList(L2);
